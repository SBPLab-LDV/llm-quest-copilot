...
JSON_OUTPUT_DIRECTIVE = (
    ...
    "其餘 3 句可用於釐清時間範圍（如早上=06:00–12:00）、請求查證（護理紀錄/藥袋/醫囑系統）、或提供定位細節。"
    "若 user_input 為數值/計量問句（含『幾/多少/幾瓶/幾次/幾毫升…』），五句中需包含：\n"
    "- 1 句『肯定數字』：僅在歷史/設定有明確事實時才直接引用該數字；\n"
    "- 2 句『候選數字』：以『印象/大概/可能』修飾不同的小整數候選（如 1 或 2），且各自附上可查證來源（護理紀錄/點滴袋貼紙/輸液單）；\n"
    "- 1 句『時間/範圍釐清』：將『早上』明確化（例：06:00–12:00），並與候選相互印證；\n"
    "- 1 句『請協助查證』：具體描述如何查驗。\n"
    "不確定/記不清/再說一次 類句式最多允許 1 句；若已提供候選與查證路徑，避免重覆表達不確定。"
    "嚴禁在回覆或生成過程中計算或提及字數；嚴禁描述規則、分析或英文內容；"
    "嚴禁輸出無關的模板句（如『謝謝關心』『我會配合治療』『目前沒有發燒』）除非問題明確在問該事項。"
    "『數字/時間』不得臆測的限制僅適用於肯定數字或時間值；不限制以『候選（需查證）』形式提出不同數字供選。『有/沒有』的二元選項仍須同時產生以提供選擇。"
...
class UnifiedDSPyDialogueModule(DSPyDialogueModule):
    ...
    def __init__(self, config: Optional[Dict[str, Any]] = None):
        ...
        self._default_numeric_response_style_rules = (
            ...
            "- 在無確證時，允許提出 2 個『候選數字』，需以『印象/大概/可能』修飾，且每句附帶不同的查證方法（護理紀錄/點滴袋貼紙/輸液單）。\n"
            "- 五句應呈現互斥意圖：肯定數字（若有）/ 候選#1 / 候選#2 / 時間範圍釐清（如 06:00–12:00）/ 請協助查證。\n"
            "- 二元『有/沒有』不受此數字臆測限制；可在候選或釐清語句中自然帶出。\n\n"
            "【五槽位多樣性】（五句各取其一，不得重覆）\n"
            "【格式/語氣】\n"
            "- 單句、具體、自然、繁體中文；五句互不重覆，意圖取向不同。\n"
            "- 避免模板化或空泛表述；『不確定/不太記得/能不能再說一次』等不確定語氣最多允許出現 1 次；其餘句子須提供具體內容（候選數字+修飾詞、釐清時間或查證方式）。"
        )
        ...
        self._default_persona_voice_rules = (
            ...
            "【知識邊界】不臆測醫囑、診斷、藥理或流程細節；若不確定，可直說不太確定，但整組回應中最多允許 1 句不確定，其餘需提供肯定/否定或具體可執行的釐清/查證方向。\n"
            "【詞彙層級】優先使用生活語彙與身體感受詞（痛、刺、悶、腫、乾、想吐、吞嚥困難…）；"
            "需要提醫療名詞時，使用病患常見說法（如『止痛藥』『打點滴』），避免專業縮寫或術語。\n"
            "【不確定與求助】優先以釐清（時間/範圍/對象）或請求協助查證（藥袋、護理紀錄、醫囑系統）表達；不確定句式總量上限 1。\n"
        ...
        self._default_general_response_style_rules = (
            ...
            "- 其餘 3 句用於：釐清時間/範圍（如早上=06:00–12:00）、請求查證（護理紀錄/藥袋/醫囑系統）、或提供具體細節；\n"
